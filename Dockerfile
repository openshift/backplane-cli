...
FROM registry.access.redhat.com/ubi9:9.6 as base
...
# Download the amd64 binary tarball (for x86_64/amd64 architecture)
RUN FILENAME=$(awk '/openshift-client-linux.*tar\.gz/ && /amd64/ {print $2; exit}' sha256sum.txt) && \
    curl -sSLf -O ${OC_URL}/${FILENAME}
