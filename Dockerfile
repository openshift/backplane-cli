FROM registry.access.redhat.com/ubi9/ubi:9.4-1123 AS base
...
### Build backplane-cli
FROM registry.access.redhat.com/ubi9/ubi:9.4-1123 AS bp-cli-builder
...
### Build dependencies
FROM registry.access.redhat.com/ubi9/ubi:9.4-1123 AS dep-builder
...
RUN curl -sSLf -O ${OC_URL}/$(awk -v asset="openshift-client-linux" '$0~asset {print $2}' sha256sum.txt | grep -v arm64 | grep -v ppc64 | grep -v amd64 | grep -v rhel)
