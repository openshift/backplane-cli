...
build: clean
	env -u GOTOOLCHAIN GOTOOLCHAIN=go1.25.3+auto go build -o ocm-backplane ./cmd/ocm-backplane || exit 1

build-static: clean
	env -u GOTOOLCHAIN GOTOOLCHAIN=go1.25.3+auto go build -a -installsuffix cgo -ldflags '-extldflags "-static"' -o ocm-backplane ./cmd/ocm-backplane || exit 1

install:
	GOTOOLCHAIN=go1.25.3+auto go install ./cmd/ocm-backplane
...
test:
	env -u GOTOOLCHAIN GOTOOLCHAIN=go1.25.3+auto go test -v $(TESTOPTS) ./...
...
cross-build-darwin-amd64:
	+@GOOS=darwin GOARCH=amd64 GOTOOLCHAIN=go1.25.3+auto go build $(GO_BUILD_FLAGS_DARWIN) -o $(CROSS_BUILD_BINDIR)/ocm-backplane_darwin_amd64 ./cmd/ocm-backplane
.PHONY: cross-build-darwin-amd64

cross-build-linux-amd64:
	+@GOOS=linux GOARCH=amd64 GOTOOLCHAIN=go1.25.3+auto go build $(GO_BUILD_FLAGS_LINUX_CROSS) -o $(CROSS_BUILD_BINDIR)/ocm-backplane_linux_amd64 ./cmd/ocm-backplane
...
.PHONY: generate
generate:
	GOTOOLCHAIN=go1.25.3+auto go generate ./...
