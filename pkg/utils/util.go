...
var (
	// For some reason, backplane is not deployed on v3 hive, as a replace, we will use the backplane on v4 hive.
	shardReplaceMap = map[string]string{
		"https://api.hive-integration.openshift.com:443": "https://api.hivei01ue1.f7i5.p1.openshiftapps.com:6443",
		"https://api.hive-production.openshift.com:443":  "https://api.hivep01ue1.b6s7.p1.openshiftapps.com:6443",
	}
)
...
func GetOCMEnvironmentFromCoonectionURL(connectionURL string) string {
	switch connectionURL {
	case "https://api.openshift.com":
		return "production"
	case "https://api.stage.openshift.com":
		return "staging"
	case "https://api.integration.openshift.com":
		return "integration"
	}
