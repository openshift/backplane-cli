...
func runElevate(cmd *cobra.Command, argv []string) error {
	e := elevate.ElevateConfig{
		GetCurrentUsername:                      elevate.GetCurrentUsername,
		AddImpersonationReasonRoleToCurrentUser: elevate.AddImpersonationReasonRoleToCurrentUser,
		AddElevationReasonToRawKubeconfig:       elevate.AddElevationReasonToRawKubeconfig,
		WriteConfigToTempKubeconfig:             elevate.WriteConfigToTempKubeconfig,
	}

	return elevate.RunElevate(e, argv)
}
