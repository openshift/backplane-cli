...
`),
	...
	Context("get a single managed job", func() {
		...
		It("Should able use the current logged in cluster if non specified and retrieve from config file", func() {
			...
			mockClient.EXPECT().GetRun(gomock.Any(), "configcluster", gomock.Eq(testJobId)).Return(fakeResp, nil)

			sut.SetArgs([]string{"get", testJobId})
			err = sut.Execute()
	...
	Context("get a all managed jobs", func() {
		...
		It("Should able use the current logged in cluster if non specified and retrieve from config file", func() {
			...
			mockOcmInterface.EXPECT().GetOCMAccessToken().Return(&testToken, nil).AnyTimes()
			mockClientUtil.EXPECT().MakeRawBackplaneAPIClient("https://api-backplane.apps.something.com").Return(mockClient, nil)
			mockClient.EXPECT().GetAllJobs(gomock.Any(), "configcluster").Return(fakeRespMultiple, nil)

			sut.SetArgs([]string{"get"})
			err = sut.Execute()
