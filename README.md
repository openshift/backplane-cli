...
As part of our continuous integration (CI) process, we've incorporated `govulncheck` to identify known vulnerabilities in the `backplane-cli` codebase. Following each CI execution, especially when PRs are submitted, a detailed vulnerability report can be found in `build-log.txt`. This is located within the `artifacts/test/` directory of the `ci/prow/scan-optional` test.
